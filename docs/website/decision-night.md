...
The Interview notes heading also names the members who wrote those notes, in smaller muted text beside the title, for example "Interview notes   Bee Booker, Kay Kimura". Names only: the private evaluations are still not copied onto the slide, which is the rule stated above and is unchanged. The list is deduplicated and sorted, covers every round including archived ones, and a rushee nobody has written about shows the bare title with no names. A member whose account was deleted keeps their name. Only the interview section carries names; the summary heading is untouched.
