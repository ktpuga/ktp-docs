...
The slide also shows how many people have responded, to whoever is running the meeting. While a ballot is open it reads `23 responses: 20 scored, 3 abstained`; once the API confirms the close it reads `Voting closed.` with those same final counts, so the result does not vanish when the timer runs out. Undecided counts as scored; Abstain is counted separately; the two always add up to the total. Changing a choice moves it between the two categories without adding a response, and a vote refused for arriving late is never counted. A fresh ballot reads `0 responses`, while an applicant whose ballot was never opened shows no counts at all, which is how the two are told apart. Counts belong to one ballot, so moving to the next applicant never shows the previous one, and reopening a ballot for the same rushee starts from zero. If the local countdown hits zero before the API has confirmed, the slide says it is waiting for the final count rather than showing a total as final, and a failed refresh marks the counts as not current. Ordinary members never see any of this: the API omits the field for them, because a running breakdown could influence ballots still being cast. There is no quorum and nothing closes automatically; the counts are for running the meeting, not a threshold.
...
The Interview notes heading also names the members who interviewed that rushee, in smaller muted text beside the title, for example "Interview notes   Bee Booker, Kay Kimura". Two sources are merged: whoever wrote a note, and whoever signed up for the slot the rushee booked, so an interviewer who sat in without writing anything still appears. Names only: the private evaluations are still not copied onto the slide, which is the rule stated above and is unchanged. The list is deduplicated and sorted, covers every round including archived ones, and a rushee with neither shows the bare title with no names. Only the interview section carries names; the summary heading is untouched.

One caveat worth knowing before a meeting. Interview slots hold one rushee by default, and for those the names are exactly right. If a slot was opened to several seats to run parallel rooms, every interviewer signed up for that block is listed for every rushee on it, because nothing records which interviewer actually saw which rushee. Withdrawing a signup removes that name; a member whose account was deleted keeps their name only if they wrote a note.
