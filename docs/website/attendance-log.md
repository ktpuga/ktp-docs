...
- The API reports both `attended`, the literal count of events the member was present at, and `credited`, which adds approved absences and is the number shown as the ratio. Anything meaning actual presence must read `attended`; anything showing a member how they stand reads `credited`.
...
**Excused** counts every completed absence the chapter approved, whether it was marked excused through check-in management or approved as an emergency waiver. It is separate from counted absences and never consumes a free absence. **Emergency waived** additionally counts the approved emergencies on their own, so the two kinds stay distinguishable: a waived event is included in Excused AND shown here. Pending events do not enter either count until check-in closes. Committee logs show Excused but do not expose private emergency decisions.

**Required events attended** counts attendance the chapter credits, which is events attended plus approved absences. An approved absence does not lower the figure: a member with four required events who attended two and had one emergency approved reads 3 / 4, not 2 / 4. Before 2026-09-23 it counted only `present` marks, so an approved emergency silently cost a required event and appeared nowhere, with Excused reading 0 beside Emergency waived 1.

This changes what is displayed, not who is in good standing. Counted absences, free absences left and the over-allowance figure are unchanged and never counted an excused or waived event in the first place.
