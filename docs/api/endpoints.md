...
| GET | `/decision-night/current` | Eligible voters: visibility, server time, active rushee name/photo/major, deadline, own selection and capability flags. While hidden, `round` and `last_vote` are null. After closure, no rushee data; own last vote confirmation only. Managers (executive board or pledge chair) additionally receive a `progress` object for the newest ballot of the current meeting phase: `round_id`, `candidate_id`, `meeting_id`, `phase`, `state` (`open` or `closed`), `closes_at`, `closed_at`, and the counts `total`, `scored` and `abstained`. It is `null` when no such ballot exists and the key is omitted entirely for ordinary voters. Counts come from that exact ballot, so changing a vote moves it between `scored` and `abstained` without raising `total`, and a refused late vote never appears. |
